...
export function markupPagination() {
  ...
  if (filmsApiServise.getPage() != 1 && window.screen.width >= 768) {
    document.querySelector('.pagination__button--button1').style.display =
      'block';
  ...
  if (
    filmsApiServise.getPage() > 3 ||
    (filmsApiServise.getPage() > 2 && window.screen.width < 768)
  ) {
  ...
  if (
    filmsApiServise.getPage() > 2 ||
    (filmsApiServise.getPage() > 1 && window.screen.width < 768)
  ) {
  ...
  if (
    filmsApiServise.getPage() + 1 < filmsApiServise.getTotalPages() ||
    (filmsApiServise.getPage() < filmsApiServise.getTotalPages() &&
  ...
  if (
    filmsApiServise.getPage() + 2 < filmsApiServise.getTotalPages() ||
    (filmsApiServise.getPage() + 1 < filmsApiServise.getTotalPages() &&
      window.screen.width < 768)
  ) {
  ...
  if (
    filmsApiServise.getPage() + 3 < filmsApiServise.getTotalPages() &&
    window.screen.width >= 768
  ) {
  ...
  if (
    ...
    document.querySelector('.pagination__button--button8').style.display =
      'block';
  } else {
  ...
  document.querySelector('.pagination__button--button3').textContent = `${
    filmsApiServise.getPage() - 2
  }`;
  document.querySelector('.pagination__button--button4').textContent = `${
    filmsApiServise.getPage() - 1
  }`;
  document.querySelector(
    '.pagination__currentPage'
  ).textContent = `${filmsApiServise.getPage()}`;
  document.querySelector('.pagination__button--button5').textContent = `${
    filmsApiServise.getPage() + 1
  }`;
  document.querySelector('.pagination__button--button6').textContent = `${
    filmsApiServise.getPage() + 2
  }`;
  document.querySelector(
    '.pagination__button--button7'
  ).textContent = `${filmsApiServise.getTotalPages()}`;
